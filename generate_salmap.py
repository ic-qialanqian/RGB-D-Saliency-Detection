    ...
    with torch.no_grad():
        for name, root in to_test.items():
            ...
            for idx, img_name in enumerate(img_list):
                ...
                prediction,mask = net(img_var,depth)
                #prediction = F.softmax(prediction,dim=1)
                #prediction = prediction[0][1]
                #prediction = to_pil(prediction.data.squeeze(0).cpu())
                #prediction = prediction.resize((w_, h_), Image.BILINEAR)
                #d = torch.sigmoid(d)
                #masked_d = torch.sigmoid(mask)
                
                #prediction= to_pil(prediction.data.squeeze(0).cpu())
                #prediction = prediction.resize((w_, h_), Image.BILINEAR)
                masked_d = to_pil(mask.data.squeeze(0).cpu())
                prediction = masked_d.resize((w_, h_), Image.BILINEAR)
